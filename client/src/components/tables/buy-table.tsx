...
export default function BoughtProductsTable() {
  ...
  if (error) {
    return (
      <Alert icon={<AlertCircle size={16} />} color="red">
        Failed to load bought products. Please try again later.
      </Alert>
    )
  }

  const products = data?.getUserActivity?.boughtProducts || []

  if (products.length === 0) {
    return (
      <Center py="xl">
        <Text c="dimmed">No bought products yet</Text>
      </Center>
    )
  }
