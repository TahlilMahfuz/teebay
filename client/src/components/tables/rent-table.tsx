...
export default function RentedProductsTable() {
  ...
  if (error) {
    return (
      <Alert icon={<AlertCircle size={16} />} color="red">
        Failed to load rented products. Please try again later.
      </Alert>
    )
  }

  const rentals = data?.getUserActivity?.rentedProducts || []

  if (rentals.length === 0) {
    return (
      <Center py="xl">
        <Text c="dimmed">No rented products yet</Text>
      </Center>
    )
  }
          ...
          {rentals.map((rental) => (
            <Table.Tr key={rental.id}>
              ...
              <Table.Td>
                <Stack gap={0}>
                  <Text size="sm">
                    {rental.product.owner.firstname} {rental.product.owner.lastname}
                  </Text>
                  <Text size="xs" c="dimmed">
                    {rental.product.owner.email}
                  </Text>
                </Stack>
              </Table.Td>
              <Table.Td>
